...
![](./assets/rl-dataflow-examples.png){fig-align="middle" height=300 .lightbox}

Reinforcement Learning (RL) for LLM Post-Training can typically be modeled as a dataflow graph, consisting of:

1. multiple models: actor, critic, reference policy, reward model, etc.
2. multiple stages: sampling trajectories, preparing experiences, training
3. multiple procedures: generation, inference, training

## Implementing Dataflow Graph as Execution Pattern

![](./assets/implement-ppo-w-device-placement.png){fig-align="middle" height=300 .lightbox}
...
From 0.2.0.post2 till now (after 0.3.0.post1), we have achieved speedup of  ~1.4x in the DAPO workload.
...
## Efficiency: Mutli-Controller

---

### Hybrid Engine

The optimal execution pattern for different procedures, e.g., training, generation, are usually different.

Instead of splitting the devices to deploy different engines separately for different procedures, causing many bubbles,
...
Generation (Prefilling & Decoding):

::: {.nonincremental}
- Tensor Parallelism: `tensor_model_parallel_size`
- Data Parallelism: Auto-inferred from `tensor_model_parallel_size` and `world_size`
:::
...
Training & Inference (Forward & Backward):

::: {.nonincremental}
- FSDP: `fsdp_size`
- (DeepSpeed) Ulysses: `ulysses_sequence_parallel_size`
...
```{#lst-verl-actor-loss .python lst-cap="The loss function definition in DataParallelPPOActor." code-line-numbers="3-8,12,20,22"}
...
```{#lst-verl-dapo-trainer-fit .python lst-cap="Simplified fit function in DAPORayTrainer." code-line-numbers="4,6,8,10,12-23"}
...
3. Parameter sharding manager for Megatron-Core V0.11 + latest version of inference engines
...
Besides, our team also integrates [the async engine based on vLLM V1 `AsyncLLM`](https://github.com/volcengine/verl/pull/1138). Kudos to [Xibin Wu](https://github.com/wuxibin89) for his great work!


# Thanks for Listening! {.center .unnumbered}